...
function legendreAssoc(l: number, absM: number, cosTheta: number): number {
...
function hydrogenPsi(n: number, l: number, m: number, r: number, theta: number, phi: number, Zeff: number): number {
...
interface Subshell { n: number; l: number; m: number; occ: number; cap: number; }

const AUFBAU: { n: number; l: number; cap: number }[] = [
  {n:1,l:0,cap:2},{n:2,l:0,cap:2},{n:2,l:1,cap:6},{n:3,l:0,cap:2},{n:3,l:1,cap:6},
  {n:4,l:0,cap:2},{n:3,l:2,cap:10},{n:4,l:1,cap:6},{n:5,l:0,cap:2},{n:4,l:2,cap:10},
  {n:5,l:1,cap:6},{n:6,l:0,cap:2},{n:5,l:2,cap:10},{n:6,l:1,cap:6},{n:7,l:0,cap:2},
  {n:6,l:2,cap:10},{n:7,l:1,cap:6},{n:5,l:3,cap:14},{n:6,l:3,cap:14},
];

function getConfig(Z: number): Subshell[] {
...
function slaterShielding(Z: number, n: number, l: number): number {
...
function getZeff(Z: number, n: number, l: number): number {
  return Math.max(1, Z - slaterShielding(Z, n, l));
}

// Orbital element from config for a given angular momentum quantum number
function getOutermostSubshell(Z: number, targetL: number): { n: number; l: number; m: number; Zeff: number } | null {
  const config = getConfig(Z);
...
function kindToL(kind: Exclude<OrbitalKind,'mix' | 'atlas'>): number {
  switch (kind) {
    case 's': return 0;
    case 'p': return 1;
    case 'd': return 2;
...
function mValues(l: number): number[] {
  const ms: number[] = [];
  for (let m = -l; m <= l; m++) ms.push(m);
  return ms;
}

// ─── Orbital sampling — proper rejection sampling on |ψ|² ───────────────────
...
const HYDROGEN_ATLAS: QuantumOrbitalSpec[] = [
  { n: 1, l: 0, m: 0, label: '(1,0,0)' },
  { n: 2, l: 0, m: 0, label: '(2,0,0)' },
  { n: 2, l: 1, m: 0, label: '(2,1,0)' },
  { n: 3, l: 0, m: 0, label: '(3,0,0)' },
  { n: 3, l: 1, m: 0, label: '(3,1,0)' },
  { n: 3, l: 1, m: 1, label: '(3,1,1)' },
  { n: 3, l: 2, m: 0, label: '(3,2,0)' },
  { n: 3, l: 2, m: 1, label: '(3,2,1)' },
  { n: 3, l: 2, m: 2, label: '(3,2,2)' },
  { n: 4, l: 0, m: 0, label: '(4,0,0)' },
  { n: 4, l: 1, m: 0, label: '(4,1,0)' },
  { n: 4, l: 1, m: 1, label: '(4,1,1)' },
  { n: 4, l: 2, m: 0, label: '(4,2,0)' },
  { n: 4, l: 2, m: 1, label: '(4,2,1)' },
  { n: 4, l: 3, m: 0, label: '(4,3,0)' },
  { n: 4, l: 3, m: 1, label: '(4,3,1)' },
];
...
const SUPERPOSITION_COMPONENTS: QuantumComponent[] = [
...
function orbitalColor(psi: number, prob: number, l: number): string {
...
function sampleSuperpositionPoint(
  components: QuantumComponent[],
  t: number,
  Zeff = 1,
): OrbitalPoint | null {
...
// ─── Camera ───────────────────────────────────────────────────────────────────
function getBasis(yaw: number, pitch: number) {
  ...
  return {
    ...
    fwd:   [fx, fy, fz] as [number,number,number],
  };
}

// ─── Solar system data ────────────────────────────────────────────────────────
// All distances in AU-like units, scaled for screen; GM in consistent units
interface Planet {
  name: string;
  color: string;
  size: number;
  ...
  M0: number;
  /** Mass relative to Sun (for n-body perturbation) */
  mass: number;
  /** Ring inner/outer radius ratio (Saturn only) */
...
const SOLAR_SYSTEM: Planet[] = [
  {
    name: 'Mercury', color: '#b8a898', size: 3.2, a: 0.387 * SCALE, e: 0.206,
    inc: 7.0, Omega: 48.3, omega: 29.1, M0: 0, mass: 1.7e-7,
  },
  {
    name: 'Venus', color: '#e8c870', size: 5.8, a: 0.723 * SCALE, e: 0.007,
    inc: 3.4, Omega: 76.7, omega: 54.9, M0: 45, mass: 2.4e-6,
  },
  {
    name: 'Earth', color: '#4a8eff', size: 6.0, a: 1.000 * SCALE, e: 0.017,
    inc: 0.0, Omega: 0, omega: 102.9, M0: 90, mass: 3.0e-6,
    moons: [{ name: 'Moon', a: 5.2, period: 27.3, size: 1.8, color: '#ccccbb' }],
  },
  {
    name: 'Mars', color: '#c86040', size: 4.5, a: 1.524 * SCALE, e: 0.093,
    inc: 1.85, Omega: 49.6, omega: 286.5, M0: 135, mass: 3.2e-7,
  },
  {
    name: 'Jupiter', color: '#d4a870', size: 14, a: 2.1 * SCALE, e: 0.049,
    inc: 1.3, Omega: 100.5, omega: 273.9, M0: 200, mass: 9.5e-4,
    moons: [
  ...
  {
    name: 'Saturn', color: '#e8d090', size: 12, a: 2.8 * SCALE, e: 0.056,
    inc: 2.49, Omega: 113.7, omega: 339.4, M0: 260, mass: 2.9e-4,
  ...
  {
    name: 'Uranus', color: '#80d8e8', size: 9, a: 3.6 * SCALE, e: 0.046,
    inc: 0.77, Omega: 74.0, omega: 96.5, M0: 300, mass: 4.4e-5,
  },
  {
    name: 'Neptune', color: '#4060e8', size: 8.5, a: 4.3 * SCALE, e: 0.010,
    inc: 1.77, Omega: 131.8, omega: 273.2, M0: 320, mass: 5.2e-5,
  },
];
...
function orbitalPosition(p: Planet, t: number): [number, number, number] {
...
export default function BlackHoleSimulation() {
  const [mode, setMode] = useState<'blackhole' | 'gravity' | 'quantum'>('blackhole');
  const [atomZ, setAtomZ] = useState(6);
  const [orbitalType, setOrbitalType] = useState<OrbitalKind>('atlas');
  const [showLabels, setShowLabels] = useState(true);
  const canvasRef  = useRef<HTMLCanvasElement>(null);
  const sim2DRef   = useRef<HTMLCanvasElement>(null);

  const S = useRef({
    yaw: 0.0, pitch: 0.0, camDist: 28,
    targetYaw: 0.0, targetPitch: 0.0,
    camPos: [0, 0, 28] as [number, number, number],
    moveSpeed: 0.5,
    keys: {} as Record<string, boolean>,
    spin: 0.9, diskTilt: 20, diskBright: 1.0, mass: 1.5,
    pointerDown: false, mouseInside: false, lastX: 0, lastY: 0,
    rays: [] as Ray3D[],
    raysPerClick: 3,
  });

  const moveForwardBy = useCallback((delta: number) => {
    const s = S.current;
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const gl = canvas.getContext('webgl', { antialias: false, alpha: false, depth: false }) as WebGLRenderingContext;
    ...
    let sceneProg: WebGLProgram, rayProg: WebGLProgram;
    try {
    ...
    const stepRays = () => {
      ...
      for (const r of s.rays) {
        ...
        if (d < rs * 0.7) { r.life = 0; continue; }
        if (r.age > 0.15) {
          const f = MASS3D / Math.max(d2 * d, 0.2);
          r.vx += -r.x * f; r.vy += -r.y * f; r.vz += -r.z * f;
          if (s.spin > 0) {
        ...
        if (mag > 0) { r.vx=r.vx/mag*SPD; r.vy=r.vy/mag*SPD; r.vz=r.vz/mag*SPD; }
        r.x+=r.vx; r.y+=r.vy; r.z+=r.vz;
        r.life -= 0.0045; r.age += 0.016;
        r.trail.push({x:r.x,y:r.y,z:r.z});
        if (r.trail.length > 120) r.trail.shift();
      }
      s.rays = s.rays.filter(r => r.life > 0 && Math.max(Math.abs(r.x),Math.abs(r.y),Math.abs(r.z)) < 700);
    };
    ...
    const drawRays = () => {
      ...
      {
        const s = S.current; const rs = s.mass*1.5; const SPD = 5.2;
        ...
        for (let i=0;i<120;i++) {
          ...
          vx+=-px*f; vy+=-py*f; vz+=-pz*f;
          if (s.spin>0) { const fd=s.spin*rs*rs/Math.max(r2,0.3); const nvx=vx-vz*fd; const nvz=vz+vx*fd; vx=nvx; vz=nvz; }
          const vm=Math.hypot(vx,vy,vz)||1;
          vx=vx/vm*SPD; vy=vy/vm*SPD; vz=vz/vm*SPD;
          const pp0=projectPoint(px,py,pz); px+=vx; py+=vy; pz+=vz;
      ...
      const aPos=gl.getAttribLocation(rayProg,'aPos'); gl.enableVertexAttribArray(aPos); gl.vertexAttribPointer(aPos,2,gl.FLOAT,false,SB,0);
      const aA=gl.getAttribLocation(rayProg,'aAlpha'); gl.enableVertexAttribArray(aA); gl.vertexAttribPointer(aA,1,gl.FLOAT,false,SB,2*F);
      const aC=gl.getAttribLocation(rayProg,'aColor'); gl.enableVertexAttribArray(aC); gl.vertexAttribPointer(aC,3,gl.FLOAT,false,SB,3*F);
      gl.uniform2fv(ul(gl,rayProg,'uRes'),[canvas.width,canvas.height]);
    ...
    const frame = () => {
      ...
      if (keys['KeyE']||keys['Space']) my+=1; if (keys['KeyQ']) my-=1;
      if (mx||my||mz) {
        const { right, up, fwd } = getBasis(s.yaw, s.pitch);
        const boost = keys['ShiftLeft']||keys['ShiftRight'] ? 3 : 1;
        const spd = s.moveSpeed * boost * 2.5;
        s.camPos = [s.camPos[0]+(right[0]*mx+up[0]*my+fwd[0]*mz)*spd, s.camPos[1]+(right[1]*mx+up[1]*my+fwd[1]*mz)*spd, s.camPos[2]+(right[2]*mx+up[2]*my+fwd[2]*mz)*spd];
      }
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, quadBuf);
      const aQ=gl.getAttribLocation(sceneProg,'aPos'); gl.enableVertexAttribArray(aQ); gl.vertexAttribPointer(aQ,2,gl.FLOAT,false,0,0);
      const { pos, rot } = getCam();
      gl.uniform2fv(ul(gl,sceneProg,'uRes'),[canvas.width,canvas.height]);
    ...
    // Events
    const onDown  = (e: PointerEvent) => { S.current.pointerDown=true; S.current.lastX=e.clientX; S.current.lastY=e.clientY; canvas.setPointerCapture(e.pointerId); canvas.style.cursor='grabbing'; };
    const onMove  = (e: PointerEvent) => { if (!S.current.pointerDown) return; S.current.targetYaw+=( e.clientX-S.current.lastX)*0.005; S.current.targetPitch=Math.max(-1.45,Math.min(1.45,S.current.targetPitch-(e.clientY-S.current.lastY)*0.005)); S.current.lastX=e.clientX; S.current.lastY=e.clientY; };
    const onEnter = (e: PointerEvent) => { S.current.mouseInside=true; S.current.lastX=e.clientX; S.current.lastY=e.clientY; canvas.style.cursor='grab'; };
    const onLeave = () => { S.current.mouseInside=false; S.current.pointerDown=false; canvas.style.cursor='default'; };
    const onUp    = () => { S.current.pointerDown=false; canvas.style.cursor='grab'; };
    ...
    const onTouchMove  = (e: TouchEvent) => { e.preventDefault(); if (e.touches.length===2) { const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY); setDistance(S.current.camDist*(lastPinch/d)); lastPinch=d; } };
    const onWheel = (e: WheelEvent) => { e.preventDefault(); setDistance(S.current.camDist+e.deltaY*0.04); };

    const RAY_COLS: [number,number,number][] = [[1,.9,.3],[.9,.4,1],[.3,.9,1],[1,.5,.1],[.4,1,.5],[1,.4,.4]];
    ...
    const spawnRays = (dir: [number,number,number]) => {
      ...
      for (let i=0;i<n;i++) {
        ...
        S.current.rays.push({ x:S.current.camPos[0],y:S.current.camPos[1],z:S.current.camPos[2], vx:jx/jm*spd,vy:jy/jm*spd,vz:jz/jm*spd, life:1,age:0, trail:[{x:S.current.camPos[0],y:S.current.camPos[1],z:S.current.camPos[2]}], color:RAY_COLS[i%RAY_COLS.length] });
      }
    };
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (tracked.includes(e.code)) { S.current.keys[e.code]=e.type==='keydown'; e.preventDefault(); }
      if (e.code==='KeyF'&&e.type==='keydown') { fireAtBH(); e.preventDefault(); }
    };
    ...
    return () => {
      ...
      gl.deleteProgram(sceneProg); gl.deleteProgram(rayProg);
      gl.deleteBuffer(quadBuf); gl.deleteBuffer(rayBuf);
  ...
  useEffect(() => {
    ...
    if (mode === 'gravity') {
      S.current.camPos=[0,80,380]; S.current.yaw=0; S.current.pitch=-0.22;
      S.current.targetYaw=0; S.current.targetPitch=-0.22;
    }
    if (mode === 'quantum') {
      S.current.camPos=[0,0,72]; S.current.yaw=0.2; S.current.pitch=-0.1;
      S.current.targetYaw=0.2; S.current.targetPitch=-0.1;
    }
    ...
    // Shared input
    const onDown  = (e: PointerEvent) => { S.current.pointerDown=true; S.current.lastX=e.clientX; S.current.lastY=e.clientY; canvas.setPointerCapture(e.pointerId); canvas.style.cursor='grabbing'; };
    const onMove  = (e: PointerEvent) => { S.current.targetYaw+=(e.clientX-S.current.lastX)*0.005; S.current.targetPitch=Math.max(-1.45,Math.min(1.45,S.current.targetPitch-(e.clientY-S.current.lastY)*0.005)); S.current.lastX=e.clientX; S.current.lastY=e.clientY; };
    const onEnter = (e: PointerEvent) => { S.current.mouseInside=true; S.current.lastX=e.clientX; S.current.lastY=e.clientY; canvas.style.cursor='grab'; };
    const onLeave = () => { S.current.mouseInside=false; S.current.pointerDown=false; canvas.style.cursor='default'; };
    const onUp    = () => { S.current.pointerDown=false; canvas.style.cursor='grab'; };
    const onWheel = (e: WheelEvent) => {
      ...
      const s=S.current;
      const { fwd } = getBasis(s.yaw, s.pitch);
      s.camPos=[s.camPos[0]+fwd[0]*e.deltaY*0.6, s.camPos[1]+fwd[1]*e.deltaY*0.6, s.camPos[2]+fwd[2]*e.deltaY*0.6];
    };
    const onKey = (e: KeyboardEvent) => {
      const tracked=['KeyW','KeyA','KeyS','KeyD','KeyQ','KeyE','Space','ShiftLeft','ShiftRight'];
      if (tracked.includes(e.code)) { S.current.keys[e.code]=e.type==='keydown'; e.preventDefault(); }
    ...
    const rng2 = () => { seed2=(seed2*1664525+1013904223)>>>0; return (seed2>>>0)/0x100000000; };
    const starfield = Array.from({length:220},()=>{
      const theta=Math.acos(1-2*rng2()), phi=2*Math.PI*rng2();
      const r=2800+rng2()*3200;
      return { x:Math.sin(theta)*Math.cos(phi)*r, y:Math.cos(theta)*r*0.8, z:Math.sin(theta)*Math.sin(phi)*r, size:0.5+rng2()*1.4, alpha:0.15+rng2()*0.55 };
    });

    // ── Gravity: planet state ──
    interface PlanetState { pos:[number,number,number]; vel:[number,number,number]; trail:{x:number;y:number;z:number}[]; }
    const planetState: PlanetState[] = SOLAR_SYSTEM.map((p,i) => {
      const pos = orbitalPosition(p, 0);
      // Velocity: approximate circular orbit tangent
      const dt = 0.1;
      const pos2 = orbitalPosition(p, dt);
      const vel: [number,number,number] = [(pos2[0]-pos[0])/dt,(pos2[1]-pos[1])/dt,(pos2[2]-pos[2])/dt];
    ...
    const frame = () => {
      ...
      for (const star of starfield) {
        ...
        ctx.beginPath(); ctx.arc(pr[0],pr[1],star.size,0,Math.PI*2); ctx.fill();
      }
      ...
      if (mode === 'gravity') {
        ...
        for (let i=-gridN;i<=gridN;i++) {
          ...
          for (let pass=0;pass<2;pass++) {
            for (let si=1;si<=steps;si++) {
              ...
              if (pass===0) { xa=xLine; za=t0*gridSize; xb=xLine; zb=t1*gridSize; }
              else { xa=t0*gridSize; za=xLine; xb=t1*gridSize; zb=xLine; }
              ...
              ctx.strokeStyle=`rgb(${r},${g},${b2})`; ctx.lineWidth=0.7;
        ...
        for (let i=0;i<SOLAR_SYSTEM.length;i++) {
          ...
          for (let ti=1;ti<st.trail.length;ti++) {
            ...
            ctx.beginPath(); ctx.moveTo(pr0[0],pr0[1]); ctx.lineTo(pr1[0],pr1[1]);
            ctx.strokeStyle=SOLAR_SYSTEM[i].color+Math.round(alpha*255).toString(16).padStart(2,'0');
            ctx.lineWidth=1; ctx.stroke();
          }
        }

        // ── Draw planets (depth sorted) ──
        interface DrawItem { p: Planet; st: PlanetState; pr: [number,number,number]; }
        ...
        if (sunPr) {
          for (let g=3;g>=0;g--) {
            ...
            ctx.fillStyle=glow; ctx.beginPath(); ctx.arc(sunPr[0],sunPr[1],60*(g+1),0,Math.PI*2); ctx.fill();
          }
          const sunGrad=ctx.createRadialGradient(sunPr[0]-5,sunPr[1]-5,0,sunPr[0],sunPr[1],22);
          sunGrad.addColorStop(0,'#fff8e8'); sunGrad.addColorStop(0.4,'#ffcc44'); sunGrad.addColorStop(1,'#ff8800');
          ctx.fillStyle=sunGrad; ctx.beginPath(); ctx.arc(sunPr[0],sunPr[1],22,0,Math.PI*2); ctx.fill();
          if (showLabels) {
            ctx.fillStyle='rgba(255,200,100,0.9)'; ctx.font='bold 11px "Courier New",monospace';
            ctx.textAlign='center'; ctx.fillText('Sun',sunPr[0],sunPr[1]-28);
          }
        }
        ...
        for (const item of drawList) {
          ...
          if (p.rings && pr) {
            ...
            ctx.rotate(-0.35);
            ctx.strokeStyle=p.rings.color; ctx.lineWidth=Math.max(1.5,sz*0.18);
            ctx.beginPath(); ctx.ellipse(0,0,sz*p.rings.outer,sz*p.rings.outer*0.35,0,0,Math.PI*2); ctx.stroke();
            ctx.strokeStyle=p.rings.color.replace('0.55','0.28'); ctx.lineWidth=Math.max(1,sz*0.10);
            ctx.beginPath(); ctx.ellipse(0,0,sz*p.rings.inner,sz*p.rings.inner*0.35,0,0,Math.PI*2); ctx.stroke();
            ctx.restore();
          }

          // Planet body
          const pg=ctx.createRadialGradient(pr[0]-sz*0.25,pr[1]-sz*0.25,0,pr[0],pr[1],sz);
          pg.addColorStop(0,lightenColor(p.color,1.4)); pg.addColorStop(0.6,p.color); pg.addColorStop(1,'#0a0a10');
          ctx.fillStyle=pg; ctx.beginPath(); ctx.arc(pr[0],pr[1],sz,0,Math.PI*2); ctx.fill();

          // Earth atmosphere
          if (p.name==='Earth') {
            ctx.strokeStyle='rgba(100,160,255,0.28)'; ctx.lineWidth=sz*0.35;
            ctx.beginPath(); ctx.arc(pr[0],pr[1],sz*1.18,0,Math.PI*2); ctx.stroke();
          }
          ...
          if (p.moons) {
            for (const moon of p.moons) {
              ...
              ctx.beginPath(); ctx.arc(mpr[0],mpr[1],ms,0,Math.PI*2); ctx.fill();
            }
          }

          // Label
          if (showLabels) {
            ctx.fillStyle=p.color+'cc'; ctx.font=`${Math.max(9,Math.min(13,sz*0.9))}px "Courier New",monospace`;
            ctx.textAlign='center'; ctx.fillText(p.name,pr[0],pr[1]-sz-5);
          }
        }
      }
      ...
      if (mode === 'quantum') {
        ...
        if (nucPr) {
          for (let g=3;g>=0;g--) {
            ...
            ctx.fillStyle=glow; ctx.beginPath(); ctx.arc(nucPr[0],nucPr[1],r,0,Math.PI*2); ctx.fill();
          }
          const nGrad=ctx.createRadialGradient(nucPr[0]-2,nucPr[1]-2,0,nucPr[0],nucPr[1],7);
          nGrad.addColorStop(0,'#f0e0ff'); nGrad.addColorStop(0.5,'#c070ff'); nGrad.addColorStop(1,'#6010aa');
          ctx.fillStyle=nGrad; ctx.beginPath(); ctx.arc(nucPr[0],nucPr[1],7,0,Math.PI*2); ctx.fill();
        }
        ...
        ctx.fillStyle='rgba(0,0,0,0.5)'; ctx.beginPath(); ctx.roundRect(lx,ly,126,48,5); ctx.fill();
        ctx.fillStyle='rgba(160,100,255,0.8)'; ctx.fillRect(lx+10,ly+10,10,10);
        ctx.fillStyle='rgba(255,80,200,0.8)';  ctx.fillRect(lx+10,ly+28,10,10);
  ...
  }, [mode, atomZ, orbitalType, showLabels]);

  // ── Fire ring ──────────────────────────────────────────────────────────────
  const fireRing = () => {
    const cols: [number,number,number][] = [[1,.9,.3],[.9,.4,1],[.3,.9,1],[1,.5,.1],[.4,1,.5],[1,.4,.4]];
    for (let i=0;i<32;i++) {
      const a=i/32*Math.PI*2, r=10;
      const px=Math.cos(a)*r, pz=Math.sin(a)*r;
      const m=Math.hypot(-px,0,-pz)||1;
      S.current.rays.push({ x:px,y:0,z:pz, vx:-px/m*5.2,vy:0,vz:-pz/m*5.2, life:1,age:0,trail:[], color:cols[i%cols.length] });
    }
  };
  ...
  const panel: React.CSSProperties = {
    ...
    padding:18, boxSizing:'border-box', color:'#999', fontFamily:'monospace',
  };
  const row: React.CSSProperties = { display:'flex',alignItems:'center',gap:6,fontSize:11,color:'#888',fontFamily:'monospace',letterSpacing:'0.06em' };
  const slider: React.CSSProperties = { width:68, accentColor:'#e8700a' };
  const val: React.CSSProperties = { color:'#e8a030',minWidth:34,fontSize:11 };
  const menuWrap: React.CSSProperties = { width:'100%',maxWidth:900,display:'flex',flexWrap:'wrap',gap:8,alignItems:'center',justifyContent:'space-between',border:'1px solid #1a0a00',borderRadius:8,padding:10,boxSizing:'border-box',marginBottom:12,background:'linear-gradient(180deg,#0a0604,#050302)' };
  const menuBtn = (active: boolean): React.CSSProperties => ({ fontSize:11,padding:'6px 14px',cursor:'pointer',borderRadius:6,fontFamily:'monospace',letterSpacing:'0.06em',border:active?'1px solid #e8700a':'1px solid #2a2a2a',color:active?'#f2a64b':'#777',background:active?'#120600':'#0b0b0b' });
  const btnBase: React.CSSProperties = { fontSize:11,padding:'4px 12px',cursor:'pointer',borderRadius:4,fontFamily:'monospace',letterSpacing:'0.06em' };

  return (
    <div style={{ width:'100%',minHeight:'100vh',background:'#000',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:16,boxSizing:'border-box' }}>
      {/* Mode menu */}
      <div style={menuWrap}>
        <div style={{color:'#c4832b',fontFamily:'"Courier New",monospace',letterSpacing:'0.18em',fontSize:12}}>SIMULATION MENU</div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <button style={menuBtn(mode==='blackhole')} onClick={()=>setMode('blackhole')}>Black Hole</button>
          <button style={menuBtn(mode==='gravity')}   onClick={()=>setMode('gravity')}>Solar System</button>
          <button style={menuBtn(mode==='quantum')}   onClick={()=>setMode('quantum')}>Quantum Model</button>
        </div>
      </div>
      ...
      {mode === 'blackhole' ? (
        <>
          <div style={{textAlign:'center',marginBottom:10}}>
            <h1 style={{margin:0,fontSize:'clamp(15px,2.8vw,24px)',fontWeight:400,letterSpacing:'0.22em',color:'#d4880a',fontFamily:'"Courier New",monospace',textTransform:'uppercase'}}>
              Schwarzschild Black Hole
            </h1>
            <p style={{margin:'4px 0 0',fontSize:11,color:'#444',letterSpacing:'0.12em',fontFamily:'monospace'}}>
              DRAG TO LOOK · SCROLL/PINCH TO ZOOM · CLICK TO FIRE PHOTONS · F KEY FIRES AT BH · WASD FLY · SPACE/E UP · Q DOWN
            </p>
          </div>

          <div style={{position:'relative',width:'100%',maxWidth:900}}>
            <canvas ref={canvasRef} style={{width:'100%',aspectRatio:'16/10',display:'block',cursor:'grab',borderRadius:6,border:'1px solid #2a1000'}} />
            {/* Crosshair */}
            <div style={{position:'absolute',left:'50%',top:'50%',width:14,height:14,marginLeft:-7,marginTop:-7,pointerEvents:'none'}}>
              <div style={{position:'absolute',left:6,top:0,width:2,height:14,background:'#c98d4a'}} />
              <div style={{position:'absolute',left:0,top:6,width:14,height:2,background:'#c98d4a'}} />
            </div>
            {/* HUD */}
            <div style={{position:'absolute',left:10,top:10,padding:'6px 8px',background:'rgba(0,0,0,0.55)',border:'1px solid #2a1000',borderRadius:6,color:'#c98d4a',fontFamily:'monospace',fontSize:10,letterSpacing:'0.08em'}}>
              <div>POS {s.camPos[0].toFixed(1)} {s.camPos[1].toFixed(1)} {s.camPos[2].toFixed(1)}</div>
              <div>YAW {s.yaw.toFixed(2)} PITCH {s.pitch.toFixed(2)}</div>
            </div>
          </div>

          <div style={{display:'flex',flexWrap:'wrap',gap:10,marginTop:10,maxWidth:900,width:'100%',justifyContent:'center',alignItems:'center'}}>
            <label style={row}>Disk tilt<input type="range" min={0} max={89} step={1} defaultValue={s.diskTilt} style={slider} onChange={e=>{S.current.diskTilt=+e.target.value;(e.target.nextSibling as HTMLElement).textContent=e.target.value+'°';}} /><span style={val}>{s.diskTilt}°</span></label>
            <label style={row}>Spin<input type="range" min={0} max={2} step={0.05} defaultValue={s.spin} style={slider} onChange={e=>{S.current.spin=+e.target.value;(e.target.nextSibling as HTMLElement).textContent=(+e.target.value).toFixed(2);}} /><span style={val}>{s.spin.toFixed(2)}</span></label>
            <label style={row}>Glow<input type="range" min={0.2} max={3} step={0.05} defaultValue={s.diskBright} style={slider} onChange={e=>{S.current.diskBright=+e.target.value;(e.target.nextSibling as HTMLElement).textContent=(+e.target.value).toFixed(1);}} /><span style={val}>{s.diskBright.toFixed(1)}</span></label>
            <label style={row}>Mass<input type="range" min={0.5} max={3} step={0.1} defaultValue={s.mass} style={slider} onChange={e=>{S.current.mass=+e.target.value;(e.target.nextSibling as HTMLElement).textContent=(+e.target.value).toFixed(1);}} /><span style={val}>{s.mass.toFixed(1)}</span></label>
            <label style={row}>Rays<input type="range" min={1} max={8} step={1} defaultValue={s.raysPerClick} style={slider} onChange={e=>{S.current.raysPerClick=+e.target.value;(e.target.nextSibling as HTMLElement).textContent=e.target.value;}} /><span style={val}>{s.raysPerClick}</span></label>
            <label style={row}>Dist<input type="range" min={6} max={80} step={1} defaultValue={s.camDist} style={slider} onChange={e=>{setDistance(+e.target.value);(e.target.nextSibling as HTMLElement).textContent=e.target.value;}} /><span style={val}>{s.camDist}</span></label>
            <button style={{...btnBase,background:'#100400',color:'#e8700a',border:'1px solid #8b3a0a'}} onClick={fireRing}>Ring shot</button>
            <button style={{...btnBase,background:'#0c0a08',color:'#c98d4a',border:'1px solid #533014'}} onClick={()=>{const s2=S.current;const bm=Math.hypot(-s2.camPos[0],-s2.camPos[1],-s2.camPos[2])||1;const d:any=[-s2.camPos[0]/bm,-s2.camPos[1]/bm,-s2.camPos[2]/bm];const n2=s2.raysPerClick,sp=5.2;const{right,up}=getBasis(s2.yaw,s2.pitch);for(let i=0;i<n2;i++){const j=(i-(n2-1)/2)*(n2>1?0.1:0);const jx=d[0]+right[0]*j+up[0]*j*0.35;const jy=d[1]+right[1]*j+up[1]*j*0.35;const jz=d[2]+right[2]*j+up[2]*j*0.35;const jm=Math.hypot(jx,jy,jz)||1;s2.rays.push({x:s2.camPos[0],y:s2.camPos[1],z:s2.camPos[2],vx:jx/jm*sp,vy:jy/jm*sp,vz:jz/jm*sp,life:1,age:0,trail:[{x:s2.camPos[0],y:s2.camPos[1],z:s2.camPos[2]}],color:[1,.7,.2]});}}}>Fire at BH (F)</button>
            <button style={{...btnBase,background:'#0a0a0a',color:'#555',border:'1px solid #2a2a2a'}} onClick={()=>{S.current.rays=[];}}>Clear</button>
          </div>
        </>
      ) : (
        <div style={{width:'100%',maxWidth:900}}>
          <div style={panel}>
            <h2 style={{margin:0,color:'#d0a060',fontWeight:400,letterSpacing:'0.16em',fontSize:16}}>
              {mode==='gravity' ? 'Solar System — Keplerian Orbits' : 'Quantum Atomic Orbitals'}
            </h2>
            <p style={{margin:'6px 0 0',lineHeight:1.6,fontSize:12}}>
              {mode==='gravity'
                ? 'Accurate elliptical orbits via Kepler\'s equation with correct inclinations, eccentricities, and orbital parameters. Moons of Jupiter and Saturn included. Spacetime curvature grid shows solar gravitational well.'
                : 'Real hydrogen-like wavefunctions ψ_nlm using associated Laguerre radial functions and real spherical harmonics. Probability density |ψ|² sampled via rejection sampling. Orbital phase shown by colour: positive lobe vs. negative lobe.'}
            </p>
            <div style={{marginTop:10,display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
              <span style={{fontSize:10,color:'#555'}}>DRAG TO ORBIT · SCROLL TO ZOOM · WASD TO FLY · SPACE/E UP · Q DOWN</span>
            </div>

            {mode==='gravity' && (
              <div style={{marginTop:10,display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                <button style={menuBtn(showLabels)} onClick={()=>setShowLabels(v=>!v)}>
                  {showLabels ? 'Labels ON' : 'Labels OFF'}
                </button>
                <button style={{...btnBase,background:'#0a0a0a',color:'#c98d4a',border:'1px solid #533014'}}
                  onClick={()=>{S.current.camPos=[0,80,380];S.current.yaw=0;S.current.pitch=-0.22;S.current.targetYaw=0;S.current.targetPitch=-0.22;}}>
                  Reset View
                </button>
              </div>
            )}
            ...
            {mode==='quantum' && (
              <>
                <div style={{marginTop:10,display:'flex',flexWrap:'wrap',gap:8,alignItems:'center'}}>
                  <label style={row}>
                    Z
                    <input type="range" min={1} max={118} step={1} value={atomZ} style={{...slider,accentColor:'#a060ff'}} onChange={e=>setAtomZ(+e.target.value)} />
                    <input type="number" min={1} max={118} value={atomZ} onChange={e=>setAtomZ(Math.max(1,Math.min(118,Math.floor(+e.target.value||1))))}
                      style={{width:52,background:'#0b0b0b',border:'1px solid #222',color:'#c0a0e0',fontFamily:'monospace',padding:'2px 4px',borderRadius:3}} />
                  </label>
                </div>
                <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:8}}>
                  {(['atlas','s','p','d','f','mix'] as OrbitalKind[]).map(k=>(
                    <button key={k} style={menuBtn(orbitalType===k)} onClick={()=>setOrbitalType(k)}>
                      {k==='atlas'
                        ? 'Hydrogen atlas'
                        : k==='mix'
                          ? 'superposition'
          ...
          <canvas ref={sim2DRef} style={{width:'100%',aspectRatio:'16/10',display:'block',marginTop:10,borderRadius:8,border:'1px solid #1a1a1a',background:'#030408'}} />
        </div>
      )}
    </div>
  );
